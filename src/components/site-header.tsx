  ...
  return (
    <header className="absolute top-0 left-0 right-0 z-50">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6 md:px-10 md:py-8">
        <Link to="/" className="font-display text-xl tracking-wide text-background md:text-2xl">
        ...
        <nav className="hidden items-center gap-8 md:flex">
          {navItems.map((i) => (
            <Link
              key={i.to}
              to={i.to}
              className="text-sm tracking-wide text-background/85 transition-colors hover:text-[var(--oro)]"
